...
(function() {
  window.app = angular.module('dailybandwidth', ['ngRoute', 'firebase', 'ui.bootstrap']).value('dailyBandwidthViewFirebaseURL', 'http://dailybandwidth.firebaseIO.com').factory('Weeks', function(angularFireCollection, dailyBandwidthViewFirebaseURL) {
    return angularFireCollection("" + dailyBandwidthViewFirebaseURL + "/weeks");
  }).factory('SettingsStore', function(angularFireCollection, dailyBandwidthViewFirebaseURL) {
    return angularFireCollection("" + dailyBandwidthViewFirebaseURL + "/settings");
  }).config([
...
(function() {
  app.HomeController = function($scope) {
    ...
    $scope.loadThisWeek = function() {
      return console.log('load this week');
    };
    $scope.loadNextWeek = function() {
      return console.log('load next week');
    };
    $scope.duplicatePreviousWeek = function() {
      return console.log('duplicate previous');
    };
...
(function() {
  app.LoginController = function($scope) {
    return console.log('from login controller');
  };

}).call(this);
(function() {
  app.LogoutController = function($scope, $route, $location, angularFireAuth) {
...
(function() {
  app.SettingsController = function($scope, $timeout, SettingsStore) {
    var url;
    url = "https://dailybandwidth.firebaseIO.com/" + $scope.user.login + "/settings";
    $scope.settingsStore = new Firebase(url);
    $scope.settingsStore.on('value', function(settings) {
    ...
    $scope.resetDefaults = function() {
      ...
      console.log($scope.settings);
      $scope.settingsStore.set($scope.settings, function(error) {
        $scope.$apply;
...
(function() {
  app.Settings = (function() {
    ...
    Settings.prototype.values = function() {
      if (this.settingsStore.first) {
        this.settings = this.settingsStore.first;
      } else {
        this.settings = this.setNew();
      }
      console.log('---------');
      console.log(this.settingsStore);
      console.log(this.settingsStore[0]);
      return this.settings;
    };

    Settings.prototype.resetAll = function() {
      if (this.settingsStore.first) {
        return this.settingsStore.remove(this.settingsStore.first);
      }
    };

    Settings.prototype.setNew = function() {
      var bandwith;
      bandwith = new app.DefaultBandwithSettings(null);
      this.settings = {
        defaultBandwith: bandwith.settings
      };
      return this.settingsStore.add(this.settings, function() {
  ...
  app.DefaultBandwithSettings = (function() {
    function DefaultBandwithSettings(settings) {
      if (settings) {
    ...
    DefaultBandwithSettings.prototype._setNew = function() {
      return this.settings = {
        monday: 0,
        tuesday: 0,
        wednesday: 0,
    ...
    return DefaultBandwithSettings;
